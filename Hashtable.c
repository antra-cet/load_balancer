		...
		int (*compare_function)(void*, void*))
{
	...
	DIE(ht->buckets== NULL, "Memory error from the hashtable's buckets!\n");
...
ht_put(hashtable_t *ht, void *key, unsigned int key_size,
	...
	if (curr == NULL) {
		...
		DIE(new_data.key == NULL, "Memory error from the new entry's key!\n");
		memcpy(new_data.key, key, key_size);

		new_data.value = malloc(value_size);
		DIE(new_data.value == NULL, "Memory error from the new entry's value!\n");
